...
//global
int nvm_latency_in_ns = 0; 
int CPU_FREQ_MHZ = 2000;
void init_nvm_emulator(){
	assert(getenv("NVM_LATENCY") != NULL);
	assert(getenv("CPU_FREQ_MHZ") != NULL);
	nvm_latency_in_ns = atoi(getenv("NVM_LATENCY"));
	CPU_FREQ_MHZ = atoi(getenv("CPU_FREQ_MHZ"));
	printf("NVM_EMULATOR INITIALIZE:\nNVM_LATENCY:%dns\nCPU_FREQ_MHZ:%dMHz\n", nvm_latency_in_ns, CPU_FREQ_MHZ);
}

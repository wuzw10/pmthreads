...
extern int nvm_latency_in_ns;
extern int CPU_FREQ_MHZ;
...
static __inline void nvm_clflush(const void *p)
{
	...
	//asm volatile(".byte 0x66; clflush %0" : "+m" (*(volatile char *)p)); //CLFLUSHOPT NOT SUPPORT ON OUR MACHINE 
	unsigned long etsc = read_tsc() + (unsigned long)(nvm_latency_in_ns*CPU_FREQ_MHZ/1000);
	while (read_tsc() < etsc) {}
}

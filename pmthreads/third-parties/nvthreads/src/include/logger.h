 ...
 public:
  ...
  void OpenMemoryLog(int dirtiedPagesCount, bool isHeap, unsigned long XactID) {
    ...
    if (log_dest == SSD) {
      ...
      sprintf(logPath, "/dev/shm/nvthreads/%d/logs", nvid);
    } else {
    ...
    //use single log file in our evaluation. or the NVM consumption will exceed the capacity of the emulated NVM. 
    //it will not affect the execution time 
    //cannot perform recovery test with thus modification. 
    sprintf(_mempages_filename, "%s/MemLog_%d_%d", logPath, 0, 0);

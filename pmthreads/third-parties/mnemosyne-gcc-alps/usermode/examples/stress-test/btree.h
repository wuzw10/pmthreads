...
unsigned long write_latency_in_ns = 100;
...
void inline clflush(char *data, int len)
{
  ...
  //for (ptr = data; ptr < data + len; ptr += CACHE_LINE_SIZE) {
...
public:
  btree();
  void TM_ATTR setNewRoot(char *);
  void TM_ATTR btree_insert(entry_key_t, char*);
  void TM_ATTR btree_insert_internal(char *, entry_key_t, char *, uint32_t);
  void  TM_ATTR btree_delete(entry_key_t);
  void TM_ATTR btree_delete_internal (entry_key_t, char *, uint32_t, entry_key_t *, bool *, page **);
  char TM_ATTR  *btree_search(entry_key_t);
...
public:
  ...
  bool TM_ATTR remove(btree* bt, entry_key_t key, bool only_rebalance = false, bool with_lock = true) {
    if (!only_rebalance) {
...
void TM_ATTR btree::setNewRoot(char *new_root) {
...
char TM_ATTR *btree::btree_search(entry_key_t key) {
  page* p = (page*)root;
...
void TM_ATTR btree::btree_insert(entry_key_t key, char* right) { //need to be string
  page* p = (page*)root;
...
void TM_ATTR btree::btree_insert_internal
...
void TM_ATTR btree::btree_delete(entry_key_t key) {
  page* p = (page*)root;
...
void TM_ATTR btree::btree_delete_internal

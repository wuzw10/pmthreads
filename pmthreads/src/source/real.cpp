...
void* (*WRAP(memcpy)) (void *dest, const void *src, size_t n);
void* (*WRAP(memmove))( void * dest, const void *src, size_t n);

size_t (*WRAP(fread))( void * ptr, size_t size, size_t count, FILE * stream );
...
#define assert(x) 
#endif
#define SET_WRAPPED(x, handle) WRAP(x) = (__typeof__(WRAP(x)))dlsym(handle, #x); assert(#x)
...
void init_real_functions() {
	...
	/*void *malloc_handle = dlopen("stdlib.a", RTLD_NOW | RTLD_GLOBAL | RTLD_NOLOAD);
	if (malloc_handle == NULL) {
		fprintf(stderr, "Unable to load libc.so\n");
		_exit(2);
	}*/
	

	//SET_WRAPPED(malloc, malloc_handle);
	//SET_WRAPPED(free, malloc_handle);
	//SET_WRAPPED(malloc_usable_size, malloc_handle);
	...
	SET_WRAPPED(memcpy, RTLD_NEXT);
	SET_WRAPPED(memmove, RTLD_NEXT);
